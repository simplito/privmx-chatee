...
Chatee is chat application that provides you full end to end encryption using the **PrivMX Endpoint**

## What do you need?

#### **MongoDB Instance**

You need a **MongoDB** Instance with a replica sets for transactions.
...
You can create subdomains by fetching the **/api/new-domain** endpoint. It returns a invitation token for the first staff user in this domain.

The first thing you have to do is to create a domain.
...
If you try to access a subdomain that does not exists it redirects you to /domain-not-found.
...
Afterwards you can generate more invitation tokens as the staff user.
...
Staff users are given all members from domain. \
Non-staff user are given only the staff members from the domain.

Chat is in real time. You can send there text messages and files up to 50 MB
