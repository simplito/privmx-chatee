...
import { MessageGroup, groupMessages } from './message';
...
export function ChatMessageList({
    ...
    useEffect(() => {
        setTimeout(() => {
            threadClient.markAsRead(username);
        }, READ_DELAY);
    }, [threadClient, username]);
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
        ...
        try {
            ...
            setFile([]);
            if (text) {
                sendMessage({ author: username, text });
            }
            if (files.length > 0) {
                files.map((file) => sendFileMessage({ author: username, file }));
            }

            // await threadClient.markAsRead(username);
        } catch (error) {
